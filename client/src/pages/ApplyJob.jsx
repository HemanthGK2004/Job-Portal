        ...
        <div className='bg-white text-black rounded-lg w-full'>
            <div className='flex justify-center md:justify-between flex-wrap gap-8 px-14 py-20 mb-6 bg-sky-50 border border-sky-400 rounded-xl' >
                ...
                className="h-24 bg-white rounded-lg p-4 mr-4 max-md:mb-4 border"/>
                    ...
                    <div className='flex flex-row flex-wrap max-md:justify-cneter gap-y-2 gap-6 items-center text-gray-600 mt-3'>
            ...
            <div className='flex flex-col lg:flex-row justify-between items-start'>
                ...
                {/* Right Section */}
                <div className='w-full lg:w-1/3 mt-8 lg:mt-0 ml-4 lg:ml-8 space-y-5'>
                    <h2 className='text-black-500 p-2 text-center font-bold text-2xl'>More Jobs from {JobData.companyId.name}</h2>
                    {jobs.filter(job =>job.id_id!==JobData._id && job.companyId._id === JobData.companyId._id)
                    .filter(job=>true).slice(0,4)
                    .map((job,index) => <JobCard key={index} job={job} className='mt-2'/>)}
                </div>
            </div>
        </div>
    </div>
